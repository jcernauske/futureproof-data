...
I built FutureProof to give students like Olivia what no brochure would: a clear picture of where each path actually leads, including which ones may not survive AI.
...
Wealthy families opt out by hiring $400/hr private consultants who do this research for them. Some schools don't even have reliable internet for "Googling." Teenagers get a 15-minute meeting and a brochure. Lack of data isn't the problem; interpretation is, and right now only money buys it.
...
**Tool calling against local data.** Gemma calls 8 MCP tools that query an Apache Iceberg lakehouse of federal education and labor data via DuckDB. Gemma selects the right tool, passes typed arguments, receives tabular results, and synthesizes a grounded response. "Ask the Guide" chains tool calls before synthesis. On the 26B model via OpenRouter, Gemma reliably honors `tool_choice` parameters. On the compact E4B model via Ollama, I built a three-tier fallback: native tool calling, content-JSON extraction, and re-prompting.
...
**MCP server.** `futureproof_server.py` exposes 8 tools to Gemma: the core school+major career query, occupation detail, O*NET task breakdown, career branch trees, reverse career-to-school search, institutional brand (AURA), regional cost-of-living, and cross-state purchasing-power comparison. Each validates typed args, queries Iceberg, and returns structured results.
...
**Function calling reliability varies by inference provider.** The 26B model on OpenRouter reliably produces structured tool calls. The E4B model via Ollama frequently embeds JSON in the content field instead. A three-tier fallback fires often enough on E4B that I log every instance to JSONL for analysis. On 26B, it almost never fires.
