# Feature: Migrate Gemma Call Sites from Pre-Inject to MCP Tool-Calling (PLACEHOLDER)

## Status: TODO — architectural follow-up

> **This is a placeholder, not a spec.** It captures an architectural inconsistency surfaced in the 2026-04-19 product scoping session: the Brightsmith pipeline delivers structured Gold-zone data, the MCP server exposes that data as callable tools, and then the backend bypasses both to pre-fetch + string-format data into Gemma prompts. We built a navigation layer and then chose not to navigate.
>
> Scope for the real spec: migrate the four highest-value call sites (boss narration, guidance, skill recs, career tiering) from pre-inject to tool-calling. Not intended to block hackathon; primarily post-hackathon cleanup, with opportunistic partial migration during the final weeks if boss narration becomes its own spec (see §3).
