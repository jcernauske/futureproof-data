...
**Executed:** 2026-04-11T04:20:33Z
**Runner:** `/tmp/verify_mcp_bea_rpp.py` (structural-invariant rules; standard `dq_runner` SQL path does not apply)
**Rules file:** `governance/dq-rules/mcp-bea-rpp.json` (16 rules, 14 P0 + 2 P1)
**Results:** `governance/dq-results/mcp-bea-rpp-20260411T042033Z.json`
...
| MCP-BEA-002 | governance.quality_tier equals partial_verification | Consistency | P0 | in_process_probe | **FAIL** | `governance.quality_tier` is None on every response — the framework's `BaseMCPServer.attach_governance` never surfaces `quality_tier`, and `FutureProofMCPServer` does not override it. See Notes. |
...
- **P1 warnings:** 0
- **P0 gate:** FAIL — blocks spec completion until MCP-BEA-002 is resolved
