...
9. **No `Imputation Flag` entity.** Per §2 Decision #8, IPEDS publishes parallel `X*`-prefixed columns indicating whether NCES bureau-imputed each value. We deliberately do not store these flags (≤1.22% prevalence per EDA Req 7; the schema-change cost outweighs the marginal completeness gain). If a future cycle shows imputation jumping above ~5% on any field, this decision should be revisited and the Imputation Flag entity added.
