...
| Promotion Provenance | The pipeline-stamped record of when this consumable row was promoted from Base. Required on every consumable row by the Brightsmith governance contract. | — | false | false |
...
| is stamped with | Finance Profile | Promotion Provenance | one-to-one | Every profile carries the consumable promote timestamp (`promoted_at`). Identical across all rows in a single promote run. |
...
The fundamental unit is **one institution in a single IPEDS fiscal cycle** — same as Base and Bronze. The current consumable load (FY2023) has **2,675 rows** — exact 1:1 promotion from Base (CON-IFP-001 conservation invariant). Grain is enforced by CON-IFP-003 (`unitid` uniqueness, P0) and the dedup grain `[unitid]`. Per spec §6, the deterministic record_id is computed via `compute_grain_id(row, ['unitid'], prefix='ifp')` — note the `ifp` prefix is distinct from the Base `ipf` prefix to keep hash namespaces clean across zones.
