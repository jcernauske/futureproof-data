...
```mermaid
flowchart LR
  S([Student input:<br/>school + free-text major]) --> INTENT[Gemma: CIP resolution]
  INTENT --> SPLIT{{FutureProof}}

  SPLIT -->|deterministic| GOLD[(Gold tables<br/>9 public datasets)]
  GOLD --> SCORES[Stats + boss outcomes<br/>ERN ROI RES GRW AURA]

  SPLIT -->|generative| GEMMA[Gemma 4<br/>via Ollama or OpenRouter]
  GEMMA -->|MCP tool calls<br/>9 of 12 tools| GOLD
  GEMMA --> WORDS[Narratives:<br/>Take, fights, Next Steps]
  GEMMA --> SKILLS[Skill pool:<br/>stat deltas]
  SKILLS -.equip + rescore.-> SCORES

  SCORES --> OUT([Build + receipts])
  WORDS --> OUT

  classDef gemma fill:#16a34a,stroke:#166534,color:#ffffff
  classDef data fill:#1e293b,stroke:#475569,color:#e2e8f0
  class GEMMA,INTENT gemma
  class GOLD,SCORES data
```
...
- **Narrative quality** — a five-axis rubric (relevance, specificity, voice, accuracy, length) scored 1–5 by **Claude Opus 4.7** as an external judge. I use Claude, not Gemma-as-judge, to avoid the "grading your own homework" critique.
...
| [`eval/README.md`](eval/README.md) | Eval harness methodology — how the scorers work, how to add a new surface, what I measure vs. what I deliberately don't. |
...
- **Localized in three languages today; quality scales by model variant.** English, Spanish, and Arabic — the top three home languages among U.S. public-school English learners. English and Spanish work reliably on both local `gemma4:e4b` and cloud `gemma-4-26b-a4b-it`. Arabic is reliable on 26B; on E4B, Arabic prose can fall back to English on some surfaces, so districts that need fully-offline Arabic should host 26B on capable hardware. Additional locales (Mandarin, Vietnamese, Tagalog) are post-hackathon work; the Gemma language-instruction layer is parameterized on locale, so adding one is data and interface work, not engineering.

- **Cloud vs. local is a hardware choice, not an architecture choice.** The product is inference-provider-agnostic — the same `gemma_client.py` swaps between Ollama and OpenRouter with one env var. The public demo runs on OpenRouter because the maintainer's hardware cannot host `gemma-4-26b-a4b-it` locally; a school with the right GPU/RAM can run 26B on Ollama and get the full multilingual, full-tool-loop experience entirely offline. Capability scales with the variant a deployment can support: E4B unlocks English and Spanish on a 16 GB Mac Mini; 26B unlocks the full multilingual and free-form chat for any deployment that can host it. See [`reports/spike_e2b_vs_e4b_2026-05-17.md`](reports/spike_e2b_vs_e4b_2026-05-17.md) for the measured E2B vs E4B comparison that informed the local default.
